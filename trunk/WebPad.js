...
{
    DocumentSelection.insertAtCursor(this.CurrEdit, strText);
    this.CurrEdit.focus();
}
...
{
    var inputElemPos = VKI_findPos(this.CurrEdit);
    WebPad.VKI_keyboard.style.top = inputElemPos[1] - ((WebPad.CurrEdit.keyboardPosition == "fixed" && !VKI_isIE && !WebPad.VKI_isMoz) ? VKI_scrollDist()[1] : 0) + this.CurrEdit.offsetHeight + 3 + "px";
...
{
    this.CurrEdit= el;
    this.CurrEdit.keyboardPosition = "absolute";
    this.VKI_keyboard.style.top = this.VKI_keyboard.style.right = this.VKI_keyboard.style.bottom = this.VKI_keyboard.style.left = "auto";
    this.VKI_keyboard.style.position = this.CurrEdit.keyboardPosition;
...
WebPad.ProcessKeypress= function(e)
{
	...
	var editorID = CurrEdit.getAttribute("UrduEditorId");
	...
	if (chr) 
	{
		...
		*/
		//if (!(chr = __charProcessor(chr, DocumentSelection.getSelection(currEdit)))) return ret;
...
WebPad.ProcessKeydown = function(e) {
    ...
    if (e.ctrlKey && (charCode == 32)) {
        WebPad.ToggleLanguage(CurrEdit);
        if (e.preventDefault)
            e.preventDefault();
        else 
            e.returnValue = false;
...
{
    this.CurrEdit = window.document.activeElement;
}
